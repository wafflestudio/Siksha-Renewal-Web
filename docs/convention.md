...
`components` _(deprecated)_

> _이 폴더는 `/app/components`로 대체되었습니다. 더 이상 이 폴더 아래에 추가로 컴포넌트를 배치하지 마세요._
...
> 3. **hook**은 _필요한 경우_ **api**의 method를 통해 필요한 data를 요청
...
> _원칙상 npm과 yarn 중 하나의 패키지 매니저만을 사용해야 하므로, `package-lock.json`과 `yarn.lock` 중 하나의 파일만 존재해야 합니다. 따라서 두 파일이 공존하는 현재의 상태는 적절치 않고, 언젠가 하나로 통일해야 합니다._
