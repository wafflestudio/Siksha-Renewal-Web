...
const MenuProvider: React.FC<{
  ...
  const setMenu = useCallback(
    (menu: MenuType) => setState((prevState) => ({ ...prevState, menu })),
    [],
  );

  const setReviews = useCallback(
    (reviews: ReviewListType) => setState((prevState) => ({ ...prevState, reviews })),
    [],
  );

  const setMenuLoading = useCallback(
    (menuLoading: boolean) => setState((prevState) => ({ ...prevState, menuLoading })),
    [],
  );

  const dispatch = useMemo(
    () => ({ setMenu, setReviews, setMenuLoading }),
    [setMenu, setReviews, setMenuLoading],
  );

  return (
    <MenuDispatchContext.Provider value={dispatch}>
      <MenuStateContext.Provider value={state}>{children}</MenuStateContext.Provider>
